...
**_Keywords_**: `Ontology` `Graph Linker` `RDF` `Text Similarity` `DE-9IM` `WKT` `SPARQL` `Apache Jena` `OAEI` etc.
...
| The log collection service is one of the most import service of the whole company, all data (including user operate logs, system logs, etc.) uploaded will be **backed up** into Data Warehouse for statistics.
...
- Produced all logs into the message queue of Kafka, which has four Brokers, three data backups, and seven days of log redundancy to improve the robustness of this service.
...
**_Keywords_**: `Blockchain` `web3.js` `Solidity` `Ethereum` `Hardhat` `Alchemy` `Infura` `DAO`.
